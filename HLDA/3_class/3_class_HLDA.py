...
N=3                   # Number of descriptors
Row_skip=200          # Skip <Row_skip> lines at the beginning (e.g. equilibration time)
...
for j in range(1,Npoints+1):
    ...
    #state 3
    f3=open('3/COLVAR','r')
    data3= np.loadtxt(f3,usecols=range(1,N+1),skiprows=Row_skip)
    mu3=np.zeros(N)
    Nstep3 = int(len(data3)/Npoints)
    for i in range(0,N):
        mu3[i] = np.mean(data3[0:j*Nstep3,i])
...
#   CALCULATE CENTROID OF THE DISTRIBUTIONS
    mutot=np.zeros(N)
    mutot = (mu1 + mu2 + mu3)/3.0

#   DEFINE BETWEEN AND INVERSE WITHIN CLASS MATRICES
    between_class    = np.zeros((N,N))
    within_class_inv = np.zeros((N,N))

    between_class    = np.outer((mu1-mutot),(mu1-mutot)) + np.outer((mu2-mutot),(mu2-mutot)) + np.outer((mu3-mutot),(mu3-mutot))
